...
(`models.ts`) לשאילתה שנשלחת ב-`q`: הבנאי של hbsearch מזהה `and`/`or`/`w/N` ומעביר
שאילתה כזו ל-dtSearch כמות שהיא — ולכן היא מוותרת על הרחבות המילים (אותיות שימוש,
כתיב מלא/חסר וכו'), והשדות `proximity`/`requireWordOrder` אינם משפיעים עליה.
מילות השאילתה נשלחות בלי גרשיים, כמו המילה הבסיסית שהבנאי עצמו מחפש.

| מדיניות | `q` שנשלח |
|---|---|
| `wordDistance` + `all` (וגם מארח ותיק) | השאילתה כפי שהיא, `proximity` מתורגם, `requireWordOrder` |
| `sameParagraph` + `all` | השאילתה כפי שהיא, `proximity` 30 בלי סדר |
| `sameSection` + `all` | `א and ב` |
| `anyWord` | `א or ב` |
| `mostWords` / `atLeast` | דיסיונקציה של כל צירופי k המילים: `(א w/30 ב) or ...`, ובטווח `sameSection` `(א and ב) or ...` |
| התאמה חלקית מעל `maximumMatchCombinations` | נדחית — `unsupportedPolicy` בתמונת החיפוש |

`k` הוא `n/2+1` ב-`mostWords` ו-`wordMatchCount` חתוך ל-`[1, n]` ב-`atLeast`, כמו במנוע
של אוצריא; `k = n` חוזר לשורת "כל המילים" של אותו טווח, ו-`k = 1` לדיסיונקציה.

קירובים (ל-dtSearch אין פסקה ואין סעיף): "באותה פסקה" הוא חלון `w/30` — המרווח הרחב
ביותר שהשירות מרשה — ו"תחת אותה כותרת" הוא המסמך כולו (`and`), כלומר רחב מהסעיף אך
היחידה היחידה שגדולה מהחלון.

אין ל-dtSearch אופרטור "לפחות k מתוך n", ופירוק לצירופים הוא הביטוי המדויק היחיד —
גם בצורת CNF מספר האיברים זהה. לכן `mostWords`/`atLeast` מתורגמים במדויק כל עוד
`C(n,k)` אינו עולה על `maximumMatchCombinations` (128 צירופים), ומעל התקרה התוסף
**דוחה** את החיפוש: `hebrewBooksMatchQuery` מחזיר `unsupported`,
`toHebrewBooksSnapshot` שם אותו ב-`unsupportedPolicy`, ו-`HebrewBooksRepository.search`
זורק אותו לפני שהבקשה יוצאת. במדור החיצוני ההודעה מוצגת כשגיאת הבקשה, ובמסך התוסף
כאזהרה לצד תוצאות אוצריא. הדיסיונקציה הפשוטה ששימשה כאן קודם החזירה גם ספרים עם מילה
אחת בלבד — כלומר חיפשה "מילה כלשהי" במקום "רוב המילים", חיפוש אחר מזה שהמשתמש ביקש.

...
בנוסף לסיווג היכולות נבדקת גרסת השירות מול `installer/dependencies.json`:

- `serverVersion` נמוך מהנדרש — אזהרת עדכון לא-חוסמת ליד מצב "מחובר".
- אין `serverVersion` כלל, או שאין בו חלק מספרי להשוואה — נחשב שירות ישן, ומוצגת אותה אזהרה בדיוק. את `serverVersion` מדווחות רק הגרסאות שהתוסף דורש, ולכן היעדרו אינו "גרסה לא ידועה" אלא "ישן". אוצריא מעדכנת את התוסף לבדו, ולכן תוסף חדש מול שירות ישן הוא מצב שגרתי.
- `serverVersion` שווה לנדרש או חדש ממנו — אין אזהרה.
...
| `proximity` | integer | 1–30 כשהוא מתורגם מ-`distance`; חלון של טווח קרבה (§11.1.1) חורג ממנו |
...
באותה צורה. החסימה היא על *מרחק בין מילים*; חלון של טווח קרבה ("באותה פסקה"/"תחת
אותה כותרת") אינו מרחק אלא תרגום של אזור, ואינו כפוף לה — ראה §11.1.1.
...
`proximityScope`, `wordMatchMode` ו-`wordMatchCount` מתורגמים ב-`hebrewBooksMatchQuery`
(`models.ts`). ההגדרה שהם נושאים במנוע של אוצריא (`otzaria_search_engine`,
`rust/src/api/search_engine.rs`): "באותה פסקה" = כל המילים במסמך אינדקס אחד — שורת
ספר — בכל סדר ובכל מרחק; "תחת אותה כותרת" = כל המילים באותו בלוק `reference`
(סעיף/פרק), גם בשורות שונות; "רוב המילים" = `n/2+1`; "לפחות X" נחתך ל-`[1, n]`.
הסף נמדד על המילים ה*ייחודיות* (מילה כפולה מתמזגת ל-clause אחד), ובכל מצב שאינו
"כל המילים" אוצריא מוותרת על הסדר ועל המרווח ומתאימה ברזולוציית הטווח — שם
`wordDistance` מתנהג כ"באותה פסקה".

ל-dtSearch אין פסקה ואין סעיף: יחידת האחזור היא המסמך, כלומר ספר שלם, והאינדקס של
hbsearch אינו נושא שום סימון מבני שאפשר לשאול עליו (`PageBreak` קיים רק ב-highlight
info שממנו נגזר מספר העמוד, לא בשפת השאילתה). היחידה היחידה שקטנה מספר היא חלון
מילים, ולכן כל טווח מתורגם לחלון.

| מדיניות | מה נשלח |
|---|---|
| `wordDistance` + `all` (וגם מארח ותיק) | השאילתה כפי שהיא, `proximity` מתורגם מ-`distance`, `requireWordOrder` |
| `sameParagraph` + `all` | השאילתה כפי שהיא, `proximity` 30 (`paragraphProximity`), בלי סדר |
| `sameSection` + `all` | השאילתה כפי שהיא, `proximity` 300 (`sectionProximity`), בלי סדר |
| `anyWord` | `א or ב` — תרגום מדויק בכל טווח |
| `mostWords` / `atLeast` | דיסיונקציה של כל צירופי `k` המילים, כל צירוף שמאלני: `((א w/30 ב) w/30 ג) or …`, ובטווח `sameSection` `w/300` |
| התאמה חלקית מעל `maximumMatchCombinations` | נדחית — `unsupportedPolicy` בתמונת החיפוש |
| התאמה חלקית שמילותיה כוללות אופרטור של המנוע | נדחית — `and`/`or`/`not`/`contains`/`xfilter`/`w/N`/`pre/N` |
| שאילתה מעל `maximumQueryCharacters` | נדחית, בכל מצב התאמה |

`k` הוא `n/2+1` ב-`mostWords` ו-`wordMatchCount` חתוך ל-`[1, n]` ב-`atLeast`, על
`n` המילים הייחודיות; `k = n` חוזר לשורת "כל המילים" של אותו טווח, ו-`k = 1`
לדיסיונקציה. "כל המילים" נשאר שאילתה רגילה בכל טווח כדי שהבנאי של hbsearch ירחיב
את המילים (אותיות שימוש, כתיב מלא/חסר וכו'); שאילתת אופרטורים מוותרת על ההרחבות,
כי הבנאי מזהה `and`/`or`/`w/N` ומעביר אותה ל-dtSearch כמות שהיא. מילות שאילתת
האופרטורים נשלחות בלי גרשיים, כמו המילה הבסיסית שהבנאי עצמו מחפש.

**מה אינו מדויק, ולאיזה כיוון:**

- חלון הפסקה (30) צר מפסקה ארוכה: שתי מילות שאילתה שרחוקות זו מזו ביותר מ-30
  מילים *באותה פסקה* לא יימצאו. הטעות לכיוון החמצה, לא לכיוון רעש.
- חלון הסעיף (300) הוא בערך עמוד דפוס. סעיף ארוך ממנו מאבד את הזוגות הרחוקים בו,
  ושני סעיפים קצרים סמוכים עלולים להיראות כסעיף אחד. הקודם — `and` על הספר כולו —
  החזיר מילים מפרקים שאינם קשורים זה לזה, כלומר הרחיב את החיפוש בשקט במקום לקרב
  אותו.
- `w/N` חל על כל צמד סמוך, ולכן שאילתה בת `n` מילים עשויה להשתרע על עד
  `(n−1)·N` מילים; החלון הוא חסם לכל צמד, לא לכל האזור.
- `mostWords`/`atLeast` מתורגמים במדויק כל עוד `C(n,k)` אינו עולה על
  `maximumMatchCombinations` (10 — נמדד מול השירות בצורת הבקשה האמיתית; מעליו
  החיפוש איטי מכדי להסתיים בתקרת הזמן של הבקשה). אין ל-dtSearch אופרטור "לפחות k מתוך n",
  ופירוק לצירופים הוא הביטוי המדויק היחיד — גם בצורת CNF מספר האיברים זהה.
  מעל התקרה התוסף **דוחה** את החיפוש: `hebrewBooksMatchQuery` מחזיר
  `unsupported`, `toHebrewBooksSnapshot` שם אותו ב-`unsupportedPolicy`,
  ו-`HebrewBooksRepository.search` זורק אותו לפני שהבקשה יוצאת. במדור החיצוני
  ההודעה מוצגת כשגיאת הבקשה, ובמסך התוסף כאזהרה לצד תוצאות אוצריא. דיסיונקציה
  פשוטה במקומה (ההתנהגות הקודמת) הייתה "מילה כלשהי" — חיפוש אחר לגמרי מזה
  שהמשתמש ביקש, ורחב ממנו.

**מה dtSearch דוחה בשקט.** בקשה שהמנוע אינו מנתח חוזרת כ-200 עם אפס תוצאות, ולכן
כל צורה כזו נמנעת מראש או נדחית בהודעה:

- שרשרת `w/N` חשופה בדיסיונקציה גדולה — לכן כל צירוף נבנה שמאלני ומסוגר במפורש
  (`proximityGroup`). נמדד: 20 צירופים של שרשרות חשופות חזרו ריקים ב-28 מ"ש, ואותם
  צירופים מסוגרים החזירו תוצאות.
- מילה של המשתמש שהיא אופרטור של המנוע (`and`, `or`, `not`, `contains`, `xfilter`,
  `w/N`, `pre/N`) בתוך שאילתת האופרטורים — נדחית לפי `dtSearchOperator`. (`to`,
  `xfirstword` ו-`xlastword` נמדדו כתקינים ואינם נדחים.)
- שאילתה ארוכה מדי — `maximumQueryCharacters`.

ב"כל המילים" הטקסט נשלח כמות שהוא, והבנאי של hbsearch הוא שקורא בו אופרטורים;
מה שהוא בונה מהם אינו באחריות התוסף.

`toHebrewBooksSnapshot` שם את השאילתה הבנויה ב-`query` (וב-`fingerprint`, כך ששינוי
מדיניות מריץ חיפוש חדש ואינו נענה מהמטמון) ואת טקסט המשתמש ב-`displayQuery`.
...
`fileId` של ספר מהמאגר האישי הוא מזהה תחליפי מספרי בן 13 ספרות שהשרת גוזר מהנתיב היחסי, בטווח שמור מ־`1000000000000` ועד `2099511627776` (לא כולל) — רחוק שלושה סדרי גודל מכל FileID של היברובוקס. שרת ישן מכניס לשדה הזה את הנתיב היחסי עצמו, ואז אין מזהה מספרי כלל:

- שורה בלי מזהה מספרי אינה נכנסת לאינדקס הקטגוריות ואינה נשלחת כתוצאה למדור החיצוני של אוצריא. `Number()` עליה הוא `NaN`, שמגיע לאוצריא כ־`null`, והשורה אינה ניתנת לפתיחה בשום מסלול.
- `totalBooks`, `totalHits` והדפדוף (`offset`, `hasMore`) של המדור החיצוני נספרים על התוצאות הניתנות לפתיחה בלבד, כדי שהמדור לא ימנה ספרים שלעולם לא יוצגו ולא ידלג על שורות בעמוד הבא.
- במסך התוסף עצמו השורה מוצגת, אך הפתיחה נעצרת בהודעה "מזהה הספר בהיברובוקס אינו תקין".

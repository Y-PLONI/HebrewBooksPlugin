...
בכל מצב שאינו "כל המילים" אוצריא מוותרת על הסדר ועל המרווח ומתאימה ברזולוציית
הטווח — שם `wordDistance` מתנהג כ"באותה פסקה".
...
`k` הוא `n/2+1` ב-`mostWords` ו-`wordMatchCount` חתוך ל-`[1, n]` ב-`atLeast`;
`k = n` חוזר לשורת "כל המילים" של אותו טווח, ו-`k = 1`

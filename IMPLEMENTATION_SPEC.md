...
| `mostWords` / `atLeast` | דיסיונקציה של כל צירופי `k` המילים: `(א w/30 ב) or …`, ובטווח `sameSection` `w/300` |
...
- `mostWords`/`atLeast` מתורגמים במדויק כל עוד `C(n,k)` אינו עולה על
  `maximumMatchCombinations` (128). אין ל-dtSearch אופרטור "לפחות k מתוך n",
